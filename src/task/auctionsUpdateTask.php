...
if ($in_values !== "") {
    ...
    SELECT DISTINCT
        b.id_bidder, 'Auction you participated has finished!', 'Check results in auctions panel.', b.id_item, UTC_TIMESTAMP
    FROM
        bid b
    WHERE
    ...
    INSERT INTO notification
        (id_recipient, title_html, body_html, id_item, created_at)
    SELECT
        i.id_creator, 'Your auction has finished!', 'Check results in auctions panel.', i.id_item, UTC_TIMESTAMP
    FROM
        item i
    WHERE
        i.id_item IN ($in_values);
